...
{	
    ...
    {	
		...
		if(proximity == ACTIVE)
		{
			 ...
			 * to the proximity distance. And check accel */
            ...
            if(acclerometer_configured == TRUE)
		    {
...
void Neutral(void){
    Pin_Vibe_Write( ROFF );
    Pin_EMiddle_Write( RON );
    Pin_EBottom_Write( ROFF );
    Pin_ETop_Write( ROFF );
}
...
********************************************************************************
* Summary:
*  Starts and initializes all the Components for the accelerometer.
*
* Parameters:
*  void
*
